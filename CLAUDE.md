...
- `Pibo/` — iOS app (bundle `fun.tiebao.co.Pibo`, SDK `iphoneos`, deployment iOS 26.2). The pet UI / activity zone / HealthKit observer pipeline / 图鉴 / 一起 / share lives here. Active feature folders: `Features/Home` — the 魔丸态 home: a **SpriteKit stage** (`Home/Stage/PiboStageScene` + `PiboStageView`) for the activity zone (themed scene + Pibo + 拍一拍 + 拔毛 + 能量收集 animations), `HomeView` chrome (打招呼文案 / 与Pibo相识第N天 / 露珠相机 / 上滑 Dashboard / 能量收集 card), `PiboDashboardView` (上滑二楼 raw health data), `PiboCameraView` (拍照 + 弹幕), the derived `PetStateStore+Mowan` API, and `PetStateStore` (still computes the legacy three stats **only** to feed the widget snapshot — the home no longer shows them). `Features/Catalog` (图鉴 + 纪念波形; still uses `PetStageView`/`PixelPet`), `Features/Together` (一起养), `Features/Pet` (sprite sequences), `Features/Onboarding` (`HealthAuthView`, 魔丸态). Active services: `Services/HealthData` (the observer pipeline), `Services/Identity`, `Services/History` (`DailySnapshot`), `Services/Logging`, `Services/Localization` (中 / EN via `AppLanguage` + `AppLocalization`).
...
- `Shared/DesignSystem/` — `LP.*` tokens (Colors, Typography, Spacing, Radius, BorderWidth, Shadow, DashPattern) and components (`LPCard`, `LPStatBar`, `LPButton`, `LPPill`, `LPStickyNote`, `LPSpeechBubble`, `LPStamp`, `LPDashedRule`) plus modifiers (`lpCard`, `lpStampedCard`, `lpDashedBorder`, `lpPaper`). **Always reach for these first** before defining one-off colors/fonts/cards. Tokens are platform-aware (watchOS compresses sizes; `lpShadow` is a no-op on watchOS).
  ...
  - **`Theme/` — 关于毛的主题** (`PiboTheme` + `PiboThemeScene` / `PiboHeadItemView`, from Figma `74:6101`): token-driven 节气/活动限定 themes. Each `PiboTheme` is pure data (`scene` sky/ground + `headItem` 毛/花 kind); `PiboThemeScene` Canvas-renders the backdrop per `terrain` (meadow/beach/platform) and `PiboHeadItemView` draws the head decoration (sprout/peachBranch/seaweed/mystery). Presets: `.demon` (魔丸 D1 黑洞+?), `.sprout`, `.peachSeason`, `.aranyaSeaBreeze`. Add a theme = add a preset, no view changes.
...
- **SpriteKit** (`SpriteView` → `PiboStageScene`): the home **activity-zone stage** — Pibo character, themed sky/ground (meadow/beach/platform), idle bob, 拍一拍 bounce, 能量收集 头顶毛 animation, 拔毛 seed drop, sparkles. This is the live home stage (it grows a lot of 2D-game-like animation); SwiftUI owns only the chrome. The `PiboTheme` design-system data feeds the scene; the Canvas `PiboThemeScene` stays only for widget/preview. SpriteKit gotcha: an `SKScene()` starts at size `.zero`, so give the scene a non-zero initial size (and a per-frame build fallback) or `didMove`/`didChangeSize` never build.
...
Done on top of this layer: the SpriteKit home stage + the direct-data 6-state machine, 拍一拍/拔毛/能量收集, the 拍照 flow, and the 上滑 Dashboard. Still TODO: real `AVCaptureSession` capture in `PiboCameraView` (currently a synthesized frame), seasonal/event 主题 unlocks beyond `.demon`/`.sprout`, and the glitch/sickness/death arc — all build on `PetStateStore`'s daily snapshots (`Services/History/DailySnapshot`).
