...
Pibo 的角色表现正在从「两张贴图 + SKAction」换成**矢量 12 状态变形**，源自设计交付包 `pibo_context`(lulu-design)。本轮落了 6 个状态：`default` / `muscle` / `pigu` / `sleep-1` / `sleep-2` / `awake`。**现在默认开着**（`PiboVectorCharacterFlag` 在没有存过偏好时返回 true），可用 `-PiboVectorCharacter` / `-PiboLegacyCharacter` 强制切换；旧的双 sprite 路径原样保留可并排比较。
...
> **共同物件硬解锁（2026-08-07）** — 决定 032 已在 iOS 与 HarmonyOS 落地，能力由 `pibo-core 0.10.0` 根据永久物件 ownership 派生：吊床开放睡眠回顾与睡醒通知；补梦风铃一次开放露珠相机与 Walk Doodle；状态观测仪开放森林中的恢复状态 Modal；铃兰灯只开放逐盏魔法点灯，并预留未来 Shadow Pibo eligibility。未拥有时入口不出现在主页，路由层还要再次校验；健康授权、原始健康归档和 `bo` 形成不得门控。恢复状态当前只诚实显示等待数据/正在校准，不输出分数；Shadow Pibo 当前没有任何用户可见入口、好友、后端、同步或隐私实现。

> **首发范围裁剪 (2026-07-30，2026-08-07 更新) — read this before the navigation text below.** `Pibo/App/PiboReleaseScope.swift` is the single place that says what the 首发 build exposes. `camera` 与独立的 `walkDoodle` 发布开关当前开启，但还必须分别获得补梦风铃的 `.dewCamera` / `.walkDoodle` capability；当前关闭的是其他 `miniGames`、历史页新版 `footprintsHistory` tab 与 `customizePibo`。**All the implementations stay** — only the entries are gated. DEBUG 启动参数仍可打开发布范围用于开发，但不会绕过共同物件 capability；只有显式 `-PiboUnlockAllCommonItems` 才提供不持久化的全物件调试 override。历史 cover 当前直接渲染带云朵睡眠卡的 `PiboHistoryView`，不套 `TabView`。
>
...
### Onboarding (Figma `6424:3262`, rebuilt after the 0801 走查)

`Features/Onboarding/HealthAuthView.swift`. Beat order is the designer's frame
order: 降落 → 未知生命体自语 ×2 → 眼睛出现(会眨眼) → 轻点五下给光 → Pibo 请求能量 ×5
→ 系统授权 → 牠们退开了(眼睛消失) → 能量说明 ×3 → 决定留下. Three structural rules
came out of that review and each is a *silent* regression — the flow still runs
and still reaches the home screen if any one of them breaks:

1. **Darkness is a black mask, never the forest's own opacity.** Fading the
   artwork washes it toward whatever is behind (grey), which is exactly why the
   走查 screenshot reads milky rather than dark. The forest stays fully opaque and
   `Color.black.opacity(maskOpacity)` does the dimming — and it sits *between*
   Pibo and the watchers, so the eyes stay lit in a pitch-black frame. That is
   the Figma layer order (背景 → Pibo → 蒙层 → 眼睛 → 气泡 → 按钮), not a detail.
2. **The watchers persist until authorization**, then retreat on the
   「牠们退开了」 beat. They used to fade out during the light taps, which also made
   the existing retreat animation invisible — it was animating an already-hidden
   layer.
3. **The speaker label is part of the bubble** (Figma `Frame 9417`): stacked 4pt
   above it, left-aligned, the group centered on screen. It reads 「未知生命体」
   until Pibo first says its own name, then 「Pibo」.

The background is a **3× PNG** (`onboarding_forest_empty`). It was authored 1×
(393×852) and that — not any layout bug — is what the 走查 called 像素比较低; a
true-vector export is impractical because the illustration carries raster grain
(the SVG comes back at 18 MB). 惊恐态 / 正常态 are vector SVGs stripped of the
grey placeholder + frame-gradient rects Figma bakes into a node export.

Rehearse any single beat without tapping through:
`-PiboOnboardingStep=plea -PiboOnboardingLine=4`, plus `-PiboOnboardingLight=N`
for the 五下给光 progression.
