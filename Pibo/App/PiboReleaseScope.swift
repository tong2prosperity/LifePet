...
/// 首发（MVP）范围开关。
///
/// 被关掉的功能**代码原样保留**，这里只收起 UI 入口 —— 恢复某项时把对应的 base
/// 改回 `true` 即可，不需要还原任何实现。把范围集中在一个文件里，是为了让"首发
...
enum PiboReleaseScope {
    /// 露珠相机 + 餐照识别链路（`PiboCameraView` / `FoodRecognitionService` /
    /// `MealDetailView`）。关闭后首页右上角不出相机按钮，餐详情不出「重拍」。
    /// 注意 `SettingsSheet` 的 DEBUG「模拟拍一张午餐」绕过相机直接喂
    /// `handlePhotoSaved`，不受此开关影响 —— 它是验证识别链路的唯一通道。
    static var camera: Bool { on(false, "-PiboEnableCamera") }

    /// 游戏场（`GameListView` 及其下全部 `MiniGameKind`，含地图涂鸦）。
