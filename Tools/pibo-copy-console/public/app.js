...
function bind() {
  ...
  if($('#import-llm')) $('#import-llm').onclick=()=>{
    modal('导入 LLM 生成的 JSONL', `<p>把 LLM 输出的 JSONL 粘贴到这里。可以包含一个 json/jsonl 代码块标记，控制台会自动去掉。</p><textarea id=\"import-source\" rows=\"15\" placeholder=\"{\"state\":\"stable\",...}\n{...}"></textarea><label class=\"import-mode\">导入方式 <select id=\"import-mode\"><option value=\"append\">追加到当前草稿（推荐）</option><option value=\"replace\">替换当前草稿（必须包含完整 27 情境）</option></select></label><p class=\"muted\">导入只更新本机草稿，不会直接修改鸿蒙。导入后请在差异页审阅并保存。</p><button id=\"confirm-import\">校验并导入草稿</button>`);
    $('#confirm-import').onclick=()=>run(async()=>{const source=$('#import-source').value;const mode=$('#import-mode').value;accept(await api('/api/import-draft',{source,mode}));$('#dialog').close();notice(`已导入 ${mode==='append'?'追加':'替换'}草稿，请检查差异后再同步。`);});
  };
