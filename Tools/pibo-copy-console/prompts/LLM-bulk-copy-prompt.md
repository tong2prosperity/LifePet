...
2. 普通单句通常 8–22 个汉字，最多两个短分句。允许“呼……”“怎么了？”这类短句，也允许少量较长句，只要三秒气泡读得完。
...
一次拍一拍可以是一个单句互动单元，也可以是一个 2–3 句微章节。一个单元表达一个完整事项；多句必须有递进，例如“发现用户运动 → 联想到自己的身体 → 得出具体疑问”。第一次双击显示第一句，冷却结束后的下一次双击显示下一句，不自动播放。一个情境可以有很多单元，运行时固定顺序轮换。

不要把三句同义称赞硬拆成微章节。sleeping.asleep 全部使用单句。stable.touchDiscovery 恰好保留三个单句，代表第一次发现触碰，不是普通池。
...
同一情境内尽量混合：直接回应、观察、身体联想、轻微误会、具体疑问、行动意愿、安静收尾。微章节约占 20%–40%，但不要为了数量硬写。用户说“跳脱一些”时增加角度跨度，仍保留清楚触发点。
...
- lines 是 1–3 个对象的数组，每个对象只能有 text，或 text 加 stages。text 不得为空、不得换行；单句可 8–22 字，微章节每句都要能读懂。
