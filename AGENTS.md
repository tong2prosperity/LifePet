...
Narrative now follows `docs/narrative/`: Pibo is an amnesiac, constant-personality tsundere creature bound by a 约定, with fragmented Souls-style storytelling and memory recovery tied to cumulative health-behaviour depth. Do not revive the old 魔丸→傲娇→伙伴 personality-stage arc or the explicit future/AI timeline. The garbled voice, flower/energy loop, glitch/sickness/离去 arc, and low-pressure tone remain; the shipped home visual is now the single `forest` theme (the old 魔丸 look was retired — the `.demon`/`.peachSeason`/`.aranyaSeaBreeze` multi-theme picker no longer exists at runtime).
