...
- `Pibo/Features/`: SwiftUI/SpriteKit feature surfaces. `Home` is the full-screen horizontally pannable SpriteKit world (`PiboStageView` / `PiboStageScene`) with 拍一拍 / 拔毛 / 能量收集 / 餐食相机 entry; `History` is the 足迹 full-screen history page; `WalkDoodle` is the first-release map doodle recorder; `Games` contains unfinished engineering inventory that does not otherwise belong to the first release; `Onboarding` handles HealthKit auth. `Catalog` / `Together` were removed 2026-06-13.
...
- `/Users/trevorlink/Project/hackathon/pibo-assets`: sibling source-of-truth repository for shared animation masters and platform exports. Its `manifest.json` carries stable animation IDs; `ios/` contains iOS runtime derivatives and `harmony/` contains HarmonyOS derivatives.
...
New narrative and copy work follows the approved decisions in `docs/narrative-rebuild/`; `docs/narrative/` remains shipped lineage until the rebuild is formally migrated. Pibo is an amnesiac young firebringer on its first independent journey, trying to remain reliable through procedure and observation. Its constant traits are serious, curious, direct, procedural, and self-respecting—not tsundere, a pet, a child, or the user's health coach. It speaks briefly and plainly; garbling comes from damaged memory/translation and may decrease, while its voice stays restrained and unsweetened. Show care through remembered details, changed plans, and consequential choices; do not use denial-of-care lines such as “才不是关心你”. Personality stays constant while judgment and action change. See `docs/narrative-rebuild/decisions/005-Pibo人物基础.md`.

Low health accumulation affects Pibo's available high-energy actions, mission preparation, and story pace—not its survival, health, memory, personality, or relationship with the user. Never use low activity, missing data, absence, or permission refusal to make Pibo suffer, decay, blame the user, or threaten departure. Progress may slow but never regresses; see `docs/narrative-rebuild/decisions/027-低健康积累对Pibo的影响.md`.

Future monetization follows `docs/narrative-rebuild/decisions/028-养成社交战斗与商业化边界.md`, but none of those commercial, social, cosmetic-store, or battle capabilities belong to the current MVP. `bo`, story progress, relationship depth, memory recovery, and return readiness cannot be purchased or accelerated with money. Paid entitlements are separate and may later cover ongoing meal-camera services, cosmetics, space expression, and advanced social tools. Basic social viewing/visiting remains free. Physical return ends the Earth cohabitation and main storyline but transitions the App to a low-frequency remote connection; never strand purchased cosmetics, social identity, or game assets after story completion.
...
The sibling HarmonyOS app lives at `/Users/trevorlink/Project/hackathon/HarmonyPibo`, the shared animation source repository at `/Users/trevorlink/Project/hackathon/pibo-assets`, and the sibling backend at `/Users/trevorlink/Project/hackathon/pibo-server`; none live inside this repo. Animation binaries never belong in `pibo-core`: keep ProRes 4444 MOV masters and other source art in `pibo-assets/source/`, publish iOS-ready derivatives under `pibo-assets/ios/`, and copy/version the selected runtime files into this App. Core returns stable semantic IDs, not filenames or absolute paths. iOS backend clients are under `Pibo/Services/Backend/`. Membership uses StoreKit 2 in `MembershipService` with products `fun.tiebao.co.Pibo.membership.monthly` and `.yearly`; local simulator testing uses the root `PiboStore.storekit`.
