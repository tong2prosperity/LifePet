...
- `Pibo/Features/`: SwiftUI screens such as `Home`, `Catalog`, `Together`, `Onboarding`, `Pet`.
...
No test target exists yet. When adding tests, create an XCTest target such as `PiboTests`, mirror source paths where practical, and name files `TypeNameTests.swift`. Prioritize pet state derivation, daily decay, HealthKit event mapping, and snapshot persistence.
