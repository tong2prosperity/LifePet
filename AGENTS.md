...
- `docs/product-strategy-202608/`: current MVP mechanics and implementation source of truth. Read `05-P0-Implementation-Status.md` for the latest cross-platform checkpoint. `product-web-prototype/`, `legacy_docs/`, and `mocks/` are historical and must not override approved decisions or current code.
...
Product, character, copy, and animation direction follows `docs/PRODUCT.md`: a virtual pet fed by real health data, with growth, companionship, relationship, and care for the user. Expressive faces and readable body motion are welcome. Narrative identity and worldbuilding support that experience; detailed feature contracts remain in `docs/narrative-rebuild/decisions/`.

Health-impact boundaries are recorded in `docs/PRODUCT.md`. Negative states are allowed, but new triggers, severity, recovery, death, irreversible loss, guilt notifications, and permission punishment are not implicitly authorized by the pet definition.

Common items are persistent behavior/capability entitlements defined by `pibo-core`, not cosmetic flags. Platform stores persist ownership and derive capabilities through Core. Raw health collection, health authorization, `bo` formation, the base food camera, Walk Doodle, and Shadow friendship must remain outside item gates. The next unowned common item appears directly in the forest as one grey target; a ripe `bo` stays on Pibo's head until the user confirms direct investment from that item's Half Sheet. Do not restore a backpack, separate unlock-reward window, or pluck gesture. Hammock ownership changes sleep review/wake behavior; decision 046 defines the chime walk-echo capability and the 1 / 3 / 6 / 10 bo item chain. Decision 043 supersedes decision 032 for Shadow eligibility.
