...
The four common items are real feature entitlements defined by `pibo-core`, not cosmetic flags: hammock → sleep review + wake notification; dream-mending chime → Dew Camera + Walk Doodle; status observer → recovery-status modal; lily-of-the-valley lamp → magic lighting + future Shadow Pibo eligibility. Platform stores persist ownership and derive capabilities through Core. Raw health collection, health authorization, and `bo` formation must remain outside these gates. Unowned entries stay absent from Home, and Shadow Pibo currently has no visible route, backend, friendship, sync, or privacy implementation; see decision 032.
