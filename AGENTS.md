...
The sibling HarmonyOS app lives at `/Users/trevorlink/Project/hackathon/HarmonyPibo`, not inside this repo. The sibling backend lives at `/Users/trevorlink/Project/hackathon/pibo-server`, not inside this repo. iOS backend clients are under `Pibo/Services/Backend/`. Membership uses StoreKit 2 in `MembershipService` with products `fun.tiebao.co.Pibo.membership.monthly` and `.yearly`; local simulator testing uses the root `PiboStore.storekit`.
