...
- `Pibo/Features/`: SwiftUI/SpriteKit feature surfaces. `Home` is the fixed portrait SpriteKit forest (`PiboStageView` / `PiboStageScene`) with Pibo、`bo` 生长/拔取和 SwiftUI chrome；旧 Studio/Gym 分区与横向漫游已经删除。`History` is the 足迹 full-screen history page; `WalkDoodle` is a retained later-iteration capability whose exact release placement is under review; `Games` contains unfinished engineering inventory and is not a current Gym page; `Onboarding` handles HealthKit auth. `Catalog` / `Together` were removed 2026-06-13.
